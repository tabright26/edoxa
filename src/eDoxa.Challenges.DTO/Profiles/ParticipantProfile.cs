        ...
        public ParticipantProfile()
        {
            this.CreateMap<Participant, ParticipantDTO>()
                ...
                .ForMember(
                    ...
                    configuration =>
                        configuration.MapFrom(participant => participant.AverageScore != null ? participant.AverageScore.ToDecimal() : (decimal?) null)
                )
                .ForMember(
                    participant => participant.Matches,
                    configuration => configuration.MapFrom(participant => participant.Matches.OrderBy(match => match.Timestamp))
                );
        }
    }
}