...
namespace eDoxa.Arena.Challenges.Api.Infrastructure.Data.Storage
{
    public static class ArenaChallengesStorage
    {
        ...
        public static IReadOnlyCollection<IChallenge> TestChallenges => Challenges.ToList();
        ...
        private static IEnumerable<IChallenge> Challenges
        {
            get
            {
                ...
                using (var csv = new CsvReader(reader))
                {
                    ...
                    int ParticipantCount(Faker faker, ChallengeState state, Entries entries)
                    {
                        return state == ChallengeState.Inscription ? new Entries(faker.Random.Int(1, entries - 1)) : entries;
                    }

                    int MatchCount(Faker faker, ChallengeState state, BestOf bestOf)
                    {
