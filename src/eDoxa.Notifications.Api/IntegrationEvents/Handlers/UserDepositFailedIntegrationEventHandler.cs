...
namespace eDoxa.Notifications.Api.IntegrationEvents.Handlers
{
    public sealed class UserDepositFailedIntegrationEventHandler : IIntegrationEventHandler<UserDepositFailedIntegrationEvent>
    {
        ...
        public UserDepositFailedIntegrationEventHandler(IUserService userService, IOptionsSnapshot<SendgridOptions> options)
        {
        ...
        {
            await _userService.SendEmailAsync(integrationEvent.UserId.ParseEntityId<UserId>(), Options.Templates.UserDepositFailed, integrationEvent);
        }
    }
}
