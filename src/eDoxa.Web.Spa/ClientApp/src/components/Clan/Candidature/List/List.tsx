...
const CandidatureList: FunctionComponent<any> = ({ actions, candidatures: { data, loading }, type, isOwner = false }) => (
  <Card>
    ...
    <CardBody>
      {loading ? <Loading /> : data && data.map((candidature: ClanCandidature, index: number) => <Item key={index} actions={actions} candidature={candidature} type={type} isOwner={isOwner}></Item>)}
    </CardBody>
  </Card>
);
