...
namespace eDoxa.Payment.IntegrationTests.Controllers
{
    ...
    {
        public StripeCustomerPaymentMethodDefaultControllerPutAsyncTest(TestHostFixture testHost, TestMapperFixture testMapper) : base(testHost, testMapper)
        {
        }
        ...
        private async Task<HttpResponseMessage> ExecuteAsync(string paymentMethodId)
        {
            return await _httpClient.PutAsJsonAsync(
                $"api/stripe/customer/payment-methods/{paymentMethodId}/default",
                new
                {
                });
        }
