// Filename: ChallengesPublishedIntegrationEventHandlerTest.cs
...
{
    [TestClass]
    public sealed class ChallengesPublishedIntegrationEventHandlerTest
    {
        [TestMethod]
        public async Task Handle_SendCommandAsync_ShouldBeInvokedExactlyOneTime()
        {
            ...
            // Act
            var handler = new ChallengesPublishedIntegrationEventHandler(mockMediator.Object);

            // Assert
            await handler.Handle(new ChallengesPublishedIntegrationEvent());

            mockMediator.Verify(mediator => mediator.Send(It.IsAny<PublishChallengesCommand>(), It.IsAny<CancellationToken>()), Times.Once);
        }
    }
}